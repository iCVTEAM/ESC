...
class DERS_XR(Dataset):
    def __init__(self, global_config, config, transform):
        ...
        self.rgb_files = sorted(glob.glob(os.path.join(*[self.data_path, rgb_key, '*.png'])))
        ...
        with open(os.path.join(self.data_path, f'{self.partition}.txt'), 'r') as f:
            data_ids = f.readlines()
        data_ids = tuple([data_id.strip('\n') for data_id in data_ids])

        self.data = list(filter(lambda d: d['RGB'].endswith(data_ids), self.data))
